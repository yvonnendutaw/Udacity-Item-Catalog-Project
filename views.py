...
def showCategoryItems(category_id):
	...
	if 'username' not in login_session or creator.id != login_session['user_id']:
		return render_template(
			'publicCategoryItem.html', items=categoryItems, category_id=category_id, creator=creator)
	else:
		return render_template(
		'categoryItems.html', items=categoryItems, category_id=category_id, category = category, creator=creator)
...
def newCategoryItem(category_id):
	...
	if login_session['user_id'] !=  category.user_id:
		...
		if request.method == 'POST':
			...
			session.commit()
			flash('New Category %s Item Successfully Created' % (newCategoryItem.name))
